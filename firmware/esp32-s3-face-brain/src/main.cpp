...
Arduino_Canvas *eyeFrame = new Arduino_Canvas(
    FACE_EYE_WIDTH, FACE_EYE_HEIGHT, leftEye);
#endif
...
constexpr int16_t EYE_CY = FACE_EYE_HEIGHT / 2 + 5;
...
void drawIris(Arduino_GFX &g, int16_t ix, int16_t iy, float pupil, bool leftEye)
{
  ...
  if (palette.robot) {
    ...
    g.drawEllipse(ix, iy, dotR + 2, int16_t(float(dotR) * 0.78f) + 2, palette.accent);
    fillEllipse(g, ix - dotR / 4, iy - dotR / 3, 9, 6, rgb(220, 250, 245));
    return;
  }
  ...
  for (uint8_t i = 0; i < 32; ++i) {
    ...
    g.drawLine(ix + int16_t(ca * (styledPupil + 2.0f)), iy + int16_t(sa * (styledPupil + 2.0f)),
               ix + int16_t(ca * (irisR - 3.0f)), iy + int16_t(sa * (irisR - 3.0f)),
  ...
  fillEllipse(g, ix - 13, iy - 18, 6, 4, rgb(245, 250, 246));
  fillEllipse(g, ix + 12, iy - 8, 3, 2, rgb(218, 235, 229));
}
